...
import CardCotizacionTablet from './cards/cotizaciontablet.jsx'
...
export default function ListaCotizacionesTablet({proporcional}) {
    ...
    const limpiar_filtros = () => {
        dispatch(set_filtro_cotizaciones_search_order_amount({pagina: 'cotizaciones', search: 0, estado: 0}))
    ...
    <div className='position-relative' style={{width: '100%', paddingLeft: 60 / proporcional, paddingRight: 60 / proporcional}}>
        <div className='d-flex' style={{width: '100%', height: 50 / proporcional, marginBottom: 25 / proporcional }}>
            <div className='d-flex justify-content-between' style={{width: '100%', height: 50 / proporcional}}>
                <div className='' style={{width: '32%', height: 'auto', marginBottom: 10 / proporcional}}>
                    <p className='mb-0' 
                        ...
                        lista_cotizaciones.map ((cotizacion, numcot) => {
                            return (
                                <div key={numcot} className='d-flex justify-content-between' 
                                    ...
                                    {
                                        cotizaciones[(2 *  numcot)] ? ( 
                                            <CardCotizacionTablet cotizacion={cotizaciones[(2 *  numcot)]} key={(2 *  numcot)} index={(2 *  numcot)} proporcional={proporcional}/>
                                        ) : null
                                    }
                                    {
                                        cotizaciones[(2 *  numcot) + 1] ? ( 
                                            <CardCotizacionTablet cotizacion={cotizaciones[(2 *  numcot) + 1]} key={(2 *  numcot) + 1} index={(2 *  numcot) + 1} proporcional={proporcional}/>
                                        ) : null
                                    }
                                </div>
                            )
